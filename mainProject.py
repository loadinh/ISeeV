...
def GPA(specTooth, disp):
    ...
    normalizedShapes = np.zeros( (40, 2, 1, 14), dtype=np.double )
    ...
    while abs(dispReduct) > thresh:
        ...
        for inst in range(13):
            ...
            normalizedShapes[:,:,0,inst] = stdMat1
    ...
    return [normalizedShapes, meanDisp]
...
#obtain normalized shapes for every thooth
for th in range(totalMouths)

specTooth = np.zeros( (landmarksPerTooth, 2, 1, totalMouths), dtype=np.double )
outShape = np.zeros( (landmarksPerTooth, 2, 1, 1), dtype=np.double )
specTooth[:,:,0,:] = originalLandmarks[:,:,2,:]
[normalizedShapes, meanDisp] = GPA(specTooth, False)
print normalizedShapes.shape
